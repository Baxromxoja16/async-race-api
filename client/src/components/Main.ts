...
const Main = {
  createCars(data: carinfo) {
    ...
    const startEngine: HTMLButtonElement = document.createElement("button");
    const stopEngine: HTMLButtonElement = document.createElement("button");
  ...
  CreateMain(data: Array<carinfo>): HTMLElement {
    ...
    let b: number = Number(localStorage.getItem("pageNum"))
      ? Number(localStorage.getItem("pageNum"))
      : 0;
    ...
    // pagination  started
    const dataSort: carinfo[][] = [[]];
    let a = 0;
    for (let i = 0; i < data.length; i++) {
      if (dataSort[a].length < 7) {
        dataSort[a].push(data[i]);
      } else if (dataSort[a].length === 7) {
  ...
  removeCar(mainCars: HTMLElement) {
    const mainCar = mainCars.querySelectorAll(
      ".main-cars"
    ) as NodeListOf<Element>;
    const cars = mainCars.querySelectorAll(".remove") as NodeListOf<Element>;
    cars.forEach((el, i) => {
      el.addEventListener("click", async () => {
        const id = Number((mainCar[i] as HTMLElement).dataset.id);
        await DeleteCar(id);
        mainRender();
      });
    });
  ...
  async getHtmlElements(
    ...
    selects.forEach((el) => {
      el.addEventListener("click", () => {
        const id = Number(
          (el.parentNode?.parentNode?.parentNode as HTMLElement).dataset.id
        );
  ...
  engine(
    data: carinfo,
    id: number,
    startEngine: HTMLButtonElement,
    stopEngine: HTMLButtonElement,
    carSpan: HTMLElement
  ) {
